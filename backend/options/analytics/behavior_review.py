...
def _sort_detector_records(records, sort_by, sort_order):
    ...
    def value(record):
        ...
        if sort_by == "category": return source.category if observation else source.primary_category
...
def build_detector_evaluation_review(*, as_of, session_date=None, dataset_id=None, detector=None,
    ...
    for record in records:
        ...
        if not isinstance(record, SurfaceEvaluationEvidence):
    ...
    for (model, status, reason), values in sorted(groups.items()):
        ...
        cells.append(dict(detector_id=model, selection_status=status, selection_reason=reason,
            ...
            outcome_status="NOT_APPLICABLE_OBSERVATION" if model == "O2" else "NOT_BOUND_TO_PROSPECTIVE_OUTCOMES"))
    models = []
    for model in ("O1", "O2", "S1", "S2"):
        ...
        models.append(dict(detector_id=model, evaluated=len(model_records),
            detected=sum(record.observation.finding_disposition == "DETECTED" if isinstance(record, SurfaceEvaluationEvidence) else True for record in model_records),
...
def build_detector_alert_review(*, dataset_id, as_of, scope="LATEST", session_date=None,
    ...
    for record in page:
        ...
        rows.append(dict(evaluation_id=str(record.evaluation_id), candidate_id=str(record.candidate_id),
            ...
            original_package=originals.get(str(record.candidate_id), dict(status="UNAVAILABLE", legs=[])),
