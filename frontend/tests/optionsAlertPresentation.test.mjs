...
test('O2 alerts render observation evidence without package actions', () => {
  ...
  assert.match(page, /row\.observation \? <>/)
  ...
  assert.match(page, /source_cutoff: time\(observation\.market_cutoff\), analyzed_at: time\(observation\.decision_at\)/)
  assert.doesNotMatch(page, /option_price: 'Not a package'/)
  assert.match(page, /title="Observation evidence"/)
  assert.match(page, /Retained alert evidence \/ Delayed marks \/ Not a fill or execution permission/)
})
