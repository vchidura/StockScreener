...
test('O2 alerts render observation evidence without package actions', () => {
  ...
  assert.match(page, /row\.management_policy\?\.technical_exit/)
  assert.match(page, /planDetail\.observation \? <>/)
  assert.match(page, /residual_iv_points/)
  assert.match(page, /Development observation \/ No package, fill or execution permission/)
})
...
test('detector columns restore shared screener fields and presets exclude transient scope', () => {
  ...
  for (const key of ['contracts', 'expiry', 'stock', 'option_price', 'volume', 'open_interest', 'iv', 'delta', 'gamma', 'theta', 'vega', 'rho', 'volume_oi', 'bid_ask', 'plan_stop', 'plan_target', 'maximum_profit']) assert.ok(keys.includes(key))
  ...
  assert.deepEqual(defaults, ['triggered_at', 'underlyer', 'contracts', 'expiry', ...model.optionMarketColumns.map(column => column.key)])
  assert.deepEqual(model.detectorColumnPresets['Package / market data'], defaults)
  assert.deepEqual(model.detectorAlertColumns.filter(column => !column.hiddenByDefault && !column.locked).map(column => column.group), ['Package', 'Package', ...model.optionMarketColumns.map(() => 'Original market data')])
  ...
  assert.equal(model.detectorAlertColumns[0].locked, true)
  for (const columns of Object.values(model.detectorColumnPresets)) assert.ok(columns.includes('triggered_at'))
  assert.deepEqual(keys.slice(-4), ['detector', 'category', 'strategy', 'details'])
  ...
  assert.deepEqual(preset.columns, ['triggered_at', 'underlyer', 'iv', 'volume'])
  let saved
...
test('history presents marked return bases and recorded plans without publishing or fabricating hits', () => {
  ...
  assert.equal((page.match(/\.\.\.planCells\(/g) || []).length, 4)
...
test('Latest and History use dataset membership with sortable category and strategy columns', () => {
  ...
  assert.match(page, /<DetectorAlertsPanel key=\{`detector-v4-\$\{activeView\}`\}/)
...
test('detector workspace restores source columns and uses current models without mandatory dataset choice', () => {
  ...
  assert.match(panel, /useColumnPreferences\('option-detector-alerts-v4', detectorAlertColumns\)/)
  ...
  assert.match(panel, /source: 'EXPLICIT_ALERT_POLICY'/)
  assert.match(daily, /Daily model results/)
  assert.match(daily, /All four models/)
...
test('all four detector names are separate from observation-only return fields', () => {
  const page = readFileSync(new URL('../src/pages/OptionsAlertsPage.tsx', import.meta.url), 'utf8')
