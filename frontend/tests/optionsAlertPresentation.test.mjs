...
test('detector columns restore shared screener fields and presets exclude transient scope', () => {
  ...
  assert.deepEqual(defaults, ['underlyer', 'contracts', 'expiry', ...model.optionMarketColumns.map(column => column.key)])
  assert.deepEqual(model.detectorColumnPresets['Package / market data'], defaults)
  assert.deepEqual(model.detectorAlertColumns.filter(column => !column.hiddenByDefault && column.key !== 'underlyer').map(column => column.group), ['Package', 'Package', ...model.optionMarketColumns.map(() => 'Original market data')])
  assert.deepEqual(model.detectorColumnPresets['Greeks / volatility'], ['underlyer', 'contracts', 'expiry', 'stock', 'option_price', 'iv', 'delta', 'gamma', 'theta', 'vega', 'rho'])
  ...
  assert.deepEqual(preset.columns, ['underlyer', 'iv', 'volume'])
  let saved
