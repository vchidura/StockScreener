...
function PerformanceWorkbench({ envelope, onOffsetChange }: {
  ...
  return <>
    <section className="options-candidate-intro"><div><span>Delayed proxy tracking</span><h2>Signal performance</h2><p>Fixed checkpoints are immutable; current simulated-close P&amp;L is refreshed by the worker from a coherent delayed package. Opening this view only reads persisted values.</p></div><div><strong>{data?.cohort === 'ALL_SIGNALS' ? 'All structured signals' : 'Raw rank-leading signals'}</strong><span>{data?.days || 14}-day history · not exact Board membership</span></div></section>
